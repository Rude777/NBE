...
def generate_covid_surrogate_negatives(data_resp, n_neg=12, negative_model='uniform', neg_seed=DEFAULT_SEED):
...
def load_covid_data(include_weak_aug=False, ternary_split_feature='log_val',
                    negative_model='uniform', neg_seed=DEFAULT_SEED):
    ...
    if include_weak_aug:
        weak_aug_clia = np.array([1.2, 1.3, 1.4, 1.6, 1.8])
        weak_aug_resp = np.array([8e-6, 1.3e-5, 2.0e-5, 3.4e-5, 5.0e-5])
        parts_clia.append(weak_aug_clia)
        parts_resp.append(weak_aug_resp)
    ...
    if include_weak_aug:
        subject_ids += list(range(200, 205))
...
def subject_cumulative_prequential_curve(df, label_col, group_col='subject_id', n_shuffles=DEFAULT_N_SHUFFLES,
                                         seed=DEFAULT_SEED, min_train_subjects=3, pair_map=None,
    ...
    for s in range(n_shuffles):
        ...
        for sid in remaining:
            train_use = list(train_subjects)
            if pair_map is not None and sid in pair_map:
                train_use = [t for t in train_use if t != pair_map[sid]]

            train_df = df[df[group_col].isin(train_use)]
...
def subject_cumulative_hierarchical_curve(df, group_col='subject_id', n_shuffles=DEFAULT_N_SHUFFLES,
                                          seed=DEFAULT_SEED, min_train_subjects=10, pair_map=None,
...
if __name__ == '__main__':
    print('=' * 60)
    print('Generating 4 main figures (J, K, M, O)...')
    print('=' * 60)
    plot_fig_j_hbv()
    plot_fig_k_covid()
    plot_fig_m_bic()
